...
调用 `sessions_send` 前，Main Agent **必须**告知用户：

> "开始审计，预计需要 5-10 分钟，请稍候"

## Session Notes 机制

**对话中产生的关键信息必须持久化**，供 Audit Agent 和下次会话使用。

路径：`data/<studentId>/session-notes.yaml`（在 Main Agent workspace 下）
...
**消费方**：
- Audit Agent：审计时通过派发协议中的 artifact 间接验证
